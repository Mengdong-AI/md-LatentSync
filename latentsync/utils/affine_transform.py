...
class AlignRestore(object):
    ...
    def restore_img(self, input_img, face, affine_matrix, landmarks=None, use_lip_mask=True):
        ...
        if use_lip_mask and landmarks is not None:
            # 使用唇形蒙版
            from .lip_mask import LipMaskGenerator
            if not hasattr(self, 'lip_mask_generator'):
                self.lip_mask_generator = LipMaskGenerator()
            
            # 对关键点应用仿射变换
            landmarks_homogeneous = np.concatenate([landmarks, np.ones((landmarks.shape[0], 1))], axis=1)
            transformed_landmarks = np.dot(landmarks_homogeneous, inverse_affine.T)
            
            # 创建唇形区域蒙版
            mask = self.lip_mask_generator.create_lip_mask(transformed_landmarks, (h_up, w_up))
            # 使用蒙版混合图像
            upsample_img = self.lip_mask_generator.blend_images(upsample_img, inv_restored, mask)
        else:
            # 使用原有的全脸混合方式
